...
int main()
{
	...
	do
	{
		...
		{//resizing needed
			...
			if (maxelements>1024)
			{
				numofblocks2 = (maxelements%1024==0)?(maxelements/1024):(maxelements/1024+1);
				blocksize2 = 1024;
			...
			copyongpu<<<numofblocks2,blocksize2>>>((Dictionary*)gpu_temp_dict,(Dictionary*)gpu_output_dict,maxelements);
			...
			copyongpu<<<numofblocks2,blocksize2>>>((Dictionary*)gpu_output_dict,(Dictionary*)gpu_temp_dict,maxelements);
		...
		if (num>1024)
		{
			numofblocks = (num%1024==0)?(num/1024):(num/1024+1);
			blocksize = 1024;
		}
		else
		{
		...
		//	scanf("%s",input_string);
			...
			char *inp1 = (char*)malloc(100*sizeof(char));
			char *inp2 = (char*)malloc(100*sizeof(char));
			char jinp[10];
			char cloop[5];
		...
		float runtime_insert = (float)(clock()-begin_time)/CLOCKS_PER_SEC;
